...
<c:if test="${!empty cassettes}">
    ...
    <table align="center" frame="border" width="1150" rules="all">
        ...
        <c:forEach items="${cassettes}" var="cassette">
            <tr>
                <td>
                    <a href="${pageContext.servletContext.contextPath}/cassettes/${cassette.title}">
                            ${cassette.title}</a>
                </td>
                <td>${cassette.director}</td>
                <td>${cassette.publisher}</td>
                <td>${cassette.release_date}</td>
                <td>
                    <c:forEach items="${cassette.genre}" var="genreType">
                        ${genreType}
                    </c:forEach>
                </td>
                <td>
                    <c:forEach items="${cassette.type}" var="typeType">
                        ${typeType}
                    </c:forEach>
                </td>
